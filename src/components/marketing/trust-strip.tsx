...
export function TrustStrip() {
  return (
    <div className='relative z-10 container mx-auto -mt-8 px-4 sm:-mt-16 sm:px-6 lg:px-12'>
      <div className='grid grid-cols-2 divide-y divide-white/5 rounded-lg bg-[#2a2a2a]/50 shadow-[0px_20px_25px_-5px_rgba(0,0,0,0.1),0px_8px_10px_-6px_rgba(0,0,0,0.1)] backdrop-blur-md sm:grid-cols-4 sm:divide-x sm:divide-y-0'>
        {TRUST_ITEMS.map((item, index) => (
          <div
            key={item.label}
            className={`intersect-once flex flex-col items-center justify-center gap-2 rounded-[inherit] bg-[#1c1b1b]/90 p-4 backdrop-blur-sm sm:gap-3 sm:p-6 intersect:motion-preset-slide-up-sm ${STAGGER_DELAY[index]}`}
          >
            <Image src={item.icon} alt='' width={22} height={22} className='size-5 sm:size-[22px]' />
            <span className='text-center text-[11px] font-semibold tracking-[1.2px] text-neutral-300 uppercase sm:text-xs'>
              {item.label}
            </span>
          </div>
        ))}
